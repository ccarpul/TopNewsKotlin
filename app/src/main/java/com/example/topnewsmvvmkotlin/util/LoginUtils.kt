...
fun firebaseErrors(error: String?, context: Context) {
    ...
    when (error) {
        context.getString(R.string.networkError) -> makeToast(
            context,
            "Please check your conection"
        )
        context.getString(R.string.formatError) -> makeToast(
            context,
            "Please check your Email format"
        )
        context.getString(R.string.emailInUse) -> makeToast(context, "Email in Use")
...
fun Task<AuthResult>.AuthResult() =
    if (this.isSuccessful) { LoginViewModel.StateLiveData.RefreshUi(Result.Success("Success")) }
    else {
